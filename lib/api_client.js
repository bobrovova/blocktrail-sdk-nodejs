...
APIClient.updateHostOptions = function(options) {
    ...
    if (options.btccom) {
        if (!options.host) {
            options.host = options.btccomhost || (options.network === 'BCC' ? 'bch-chain.api.btc.com' : 'developer-btc-chain.api.btc.com');
        }
        ...
        if (!options.endpoint) {
            options.endpoint = options.btccomendpoint || ("/" + options.apiKey);
    } else {
        if (!options.host) {
